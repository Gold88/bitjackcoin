...
{
    QString ret = QString("bitjackcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BJA unit input in lnReqAmount, we generate the URI with BJA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitjackcoinUnits::format(BitjackcoinUnits::BJA, ui->lnReqAmount->valueAsMpq()));
            paramCount++;
        }
        else
        {
